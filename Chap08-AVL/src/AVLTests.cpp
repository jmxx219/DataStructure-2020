...
TEST(AVL_recursiveimpl_parentlink, DISABLED_initTest)
{
	AVL tree;
...
TEST(AVL_recursiveimpl_parentlink, DISABLED_addTest_singleRotation_rebalanceRight)
{
	AVL tree;
	tree.add(10);
	tree.add(5);
	...
	for(auto node: list){
		...
		int rightHeight{(node->left)? node->left->height: -1};
		ASSERT_TRUE(std::abs(rightHeight-leftHeight)<=1);
	}
}

TEST(AVL_recursiveimpl_parentlink, DISABLED_addTest_singleRotation_rebalanceLeft)
{
	AVL tree;
	tree.add(2);
	tree.add(5);
...
TEST(AVL_recursiveimpl_parentlink, addTest_doubleRotation_rebalanceRight)
{
	AVL tree;
	tree.add(10);
	tree.add(5);
...
TEST(AVL_recursiveimpl_parentlink, DISABLED_addTest_doubleRotation_rebalanceLeft)
{
	AVL tree;
	tree.add(10);
	tree.add(5);
...
TEST(AVL_recursiveimpl_parentlink, DISABLED_randomTest){
	AVL tree;
	int size = 0;
	while(size<32){
		int key{getRandomInt(100)};
		tree.add(key);
		size = tree.numberOfNodes();
	}
	std::vector<AVL::TreeNode*> list(tree.getAllNodes());
	for(auto node: list){
		int leftHeight{(node->left)? node->left->height: -1};
