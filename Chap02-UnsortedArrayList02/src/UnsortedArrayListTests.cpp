...
TEST(UnsortedArrayListWithDuplicateOrderPreserving, removeAllTest){
   ...
   list.removeAll(3);	//1,6,1,1,4,5,4,4,
   ASSERT_EQ(list.getSize(),8);
   std::string output = "";
   for(auto i: list)
      output += std::to_string(i)+",";
   ASSERT_EQ(output,"1,6,1,1,4,4,5,4,");
   list.removeAll(1);	//4,6,5,4,4,
   ASSERT_EQ(list.getSize(),5);
   output = "";
   for(auto i: list)
      output += std::to_string(i)+",";
   ASSERT_EQ(output,"4,6,5,4,4,");
   list.removeFirst(5);
   ASSERT_EQ(list.getSize(),4);
   list.removeAll(4);
   ASSERT_EQ(list.getSize(),1);
