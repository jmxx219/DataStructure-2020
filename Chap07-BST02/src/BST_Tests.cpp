...
TEST(BST_recursiveimpl_noparentlink, Big5)
{
	...
	tree3 = tree1;
	ASSERT_EQ(tree3.getHeight(), tree1.getHeight());
	ASSERT_EQ(tree3.numberOfNodes(), tree1.numberOfNodes());
	...
	ASSERT_EQ(tree5.getHeight(), tree1.getHeight());
	ASSERT_EQ(tree5.numberOfNodes(), tree1.numberOfNodes());
	...
	ASSERT_EQ(tree3.getHeight(), tree2.getHeight());
	ASSERT_EQ(tree3.numberOfNodes(), tree2.numberOfNodes());
	tree3.setIteratorType(BST::TreeTraversal::PREORDER);
	output3 = "";
	for(auto i: tree3)
		output3 += std::to_string(i)+",";
